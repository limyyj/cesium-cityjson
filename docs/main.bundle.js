...
var CityGMLService = /** @class */ (function () {
    ...
    CityGMLService.prototype.projectPtsToWGS84 = function (coords) {
        var projcoords = Object(__WEBPACK_IMPORTED_MODULE_2_proj4__["a" /* default */])(this.epsg, "WGS84", [(coords[0] / this.crs["xyz"]["x"]), (coords[1] / this.crs["xyz"]["y"])]);
        var newcoords = [(projcoords[0]) + this.crs["anchor"][0],
    ...
    CityGMLService.prototype.getEnginCRS = function (node) {
        var engincrs = { id: undefined, anchor: [0, 0, 0], xyz: { x: 1, y: 1, z: 1 } };
        while (node !== null) {
            var nodename = this.getName(node);
            // find EngineeringCRS
            if (nodename === "EngineeringCRS") {
                engincrs["id"] = node.getAttributeNS("http://www.opengis.net/gml", "id");
                var axes = node.getElementsByTagNameNS("http://www.opengis.net/gml", "CoordinateSystemAxis");
                for (var _i = 0, axes_1 = axes; _i < axes_1.length; _i++) {
                    var axis = axes_1[_i];
                    var units = axis.getAttributeNS("http://www.opengis.net/gml", "uom");
                    var val = 1;
                    ...
                    var direction = axis.getElementsByTagNameNS("http://www.opengis.net/gml", "axisDirection")[0].textContent;
                    if (direction === "X") {
                        engincrs["xyz"]["x"] = val;
                    }
                    else if (direction === "Y") {
                ...
                var anchorPoint = node.getElementsByTagNameNS("http://www.opengis.net/gml", "anchorPoint");
                var pt = anchorPoint[0].textContent.split(" ");
    ...
    CityGMLService.prototype.checkCityModel = function (node) {
        ...
        while (node !== null) {
            ...
            if (nodename === "name") {
                ...
                var crs = this.getEnginCRS(this.nextElement(node.firstChild));
                engincrs[crs["id"]] = crs;
            }
        ...
        if (citymodel_props["srsName"] !== undefined && citymodel_props["srsName"].startsWith("#")) {
            var srs = citymodel_props["srsName"].slice(1, citymodel_props["srsName"].length);
    ...
    CityGMLService.prototype.check = function (nodes, props) {
        ...
        for (var _i = 0, nodes_1 = nodes; _i < nodes_1.length; _i++) {
            ...
            while (node !== null) {
                ...
                // check
                var action = this.rules[sibling_name];
            ...
            if (gen_nodes.length > 0) {
                for (var _a = 0, gen_nodes_1 = gen_nodes; _a < gen_nodes_1.length; _a++) {
                    var srf = gen_nodes_1[_a];
                    ;
...
module.exports = "<div id=\"about\"  style=\"background-color: rgba(20,20,20,0.9);height: 100%;overflow-y:overlay;\"  >\r\n\t<div>\r\n\t\t<h3>cesium-viewer</h3>\r\n\t\t<p>Version 0.2.2</p>\r\n\t\t<p>This is a viewer for CityGML and CityJSON based on <a href=\"https://cesiumjs.org/\">Ceisum</a>. This project was generated with <a href=\"https://github.com/angular/angular-cli\">Angular CLI</a>.</p>\r\n\t\t<p>It aims to open and view models with the ability to:</p>\r\n\t\t<ul>\r\n\t\t\t<li>View models on a map (if location is provided)</li>\r\n\t\t\t<li>Select individual objects or surfaces and view attributes</li>\r\n\t\t\t<li>Filter visible objects based on extracted attributes</li>\r\n\t\t</ul>\r\n\t\t<p>Github Repository can be found <a href=\"https://github.com/limyyj/cesium-cityjson\">here</a>.</p>\r\n\t</div>\r\n\t<hr>\r\n\t<div>\r\n\t\t<h3>Examples</h3>\r\n\t\t<p>You may load your own model by clicking on the file button in the top right corner. If not, some examples can be loaded from the selection below.</p>\r\n\t\t<h4>Rotterdam</h4> <button id=\"rotterdam.json\" (click)=\"handleExamples($event.target.id)\">Load Model</button>\r\n\t\t<p>CityJSON example file from the <a href=\"https://www.cityjson.org/en/0.8/datasets/\">CityJSON example datasets</a>.</p>\r\n\t\t<h4>Noise ADE</h4> <button id=\"noise.gml\" (click)=\"handleExamples($event.target.id)\">Load Model</button>\r\n\t\t<p>CityGML example file containing sample data from the Noise ADE extension. Obtained from <a href=\"http://schemas.opengis.net/citygml/examples/2.0/ade/noise-ade/\">OpenGIS Schema Repository</a></p>\r\n\t</div>\r\n</div>\r\n  "
...
var AboutComponent = /** @class */ (function () {
    ...
    AboutComponent.prototype.handleExamples = function (id) {
        var _this = this;
        var path = "./assets/examples/" + id;
        ...
        readfile.then(function (file) {
            var data = _this.parseFile(file);
            _this.dataService.setGsModel(data);
            document.getElementById("overlay").style.display = "block";
            document.getElementById("loader").style.display = "block";
        });
    };
    AboutComponent.prototype.parseFile = function (file) {
        var filetype = file.type;
        var data;
        try {
            var js_data = JSON.parse(file);
            data = js_data;
        }
        catch (_a) {
            var parser = new DOMParser();
