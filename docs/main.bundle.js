...
var DataService = /** @class */ (function () {
    ...
    DataService.prototype.projectPtsToWGS84 = function (coords, proj_epsg) {
        var projcoords = proj_epsg([coords[0], coords[1]]);
        ...
        if (file !== undefined) {
            ...
            if (file["appearance"]["materials"] !== undefined) {
                file["appearance"]["materials"].forEach(function (mat) {
                    ...
                    materials_1.push(new ((_a = Cesium.Color).bind.apply(_a, [void 0].concat(color)))());
                    var _a;
                });
            }
            ...
            for (var obj_index = 0; obj_index < city_object_keys.length; obj_index++) {
                ...
                if (1) {
                    ...
                    for (var geom_index = 0; geom_index < obj["geometry"].length; geom_index++) {
                        ...
                        var _loop_1 = function (srf_index) {
                            var rings = boundaries[srf_index];
                            var extRing = rings[0];
                            var extRing_points = [];
                            ...
                            extRing.forEach(function (pt_index) {
                                extRing_points.push(vertices_1[pt_index][0], vertices_1[pt_index][1], vertices_1[pt_index][2]);
                            });
                            ...
                            if (rings.length > 0) {
                                ...
                                var _loop_2 = function (ring_index) {
                                    var temp_pts = [];
                                    rings[ring_index].forEach(function (pt_index) {
                                        temp_pts.push(vertices_1[pt_index][0], vertices_1[pt_index][1], vertices_1[pt_index][2]);
                                    });
                                ...
                                for (var ring_index = 1; ring_index < rings.length; ring_index++) {
                                    _loop_2(ring_index);
                                }
                            ...
                            if (mats !== undefined && mats[srf_index] !== null) {
                                colour = materials_1[mats[srf_index]];
                            }
                            ...
                            // }
                            // Create property bag
                            var property_bag = new Cesium.PropertyBag();
...
var ViewerComponent = /** @class */ (function (_super) {
    ...
    ViewerComponent.prototype.LoadData = function (data) {
        if (this.data !== undefined) {
            ...
            /////// LOADING DATA ///////
            this.data = data;
            var dataSource_1 = this.dataService.genCityJSONGeom(this.data);
            // const promise = Cesium.GeoJsonDataSource.load(this.data);
            viewer_1.dataSources.add(dataSource_1);
            var _HeightKey = [];
            /////// ALL THE STUFF BELOW IS FOR GENERATING HEIGHTS AND KEY BARS FROM PROPERTIES ///////
            var entities = dataSource_1["entities"]["values"];
            var self_1 = this;
